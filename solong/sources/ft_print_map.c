...
/*   Updated: 2021/12/13 10:40:39 by dso              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_loadimg(t_struct *game)
{
	game->img->wall = mlx_xpm_file_to_image(game->mlx,
			WALL, &game->img->width, &game->img->height);
	game->img->ground = mlx_xpm_file_to_image(game->mlx,
			GROUND, &game->img->width, &game->img->height);
	game->img->col = mlx_xpm_file_to_image(game->mlx,
			COL, &game->img->width, &game->img->height);
	game->img->exit = mlx_xpm_file_to_image(game->mlx,
			EXIT, &game->img->width, &game->img->height);
	game->img->player[0] = mlx_xpm_file_to_image(game->mlx,
			PLAYER0, &game->img->width, &game->img->height);
	game->img->player[1] = mlx_xpm_file_to_image(game->mlx,
			PLAYER1, &game->img->width, &game->img->height);
	game->img->player[2] = mlx_xpm_file_to_image(game->mlx,
			PLAYER2, &game->img->width, &game->img->height);
	game->img->player[3] = mlx_xpm_file_to_image(game->mlx,
			PLAYER3, &game->img->width, &game->img->height);
...
static void	ft_print_player(t_struct *game, int x, int y)
{
	mlx_put_image_to_window (game->mlx, game->win,
		game->img->ground, SPRITE_SIZE * y, SPRITE_SIZE * x);
	mlx_put_image_to_window (game->mlx, game->win,
		game->img->player[game->player->direction],
		SPRITE_SIZE * y, SPRITE_SIZE * x - 64);
}
...
static int	ft_print_loop(t_struct *game, int x, int y)
{
	if (game->map->map[x][y] == '0')
		...
		mlx_put_image_to_window (game->mlx, game->win,
			game->img->wall, SPRITE_SIZE * y, SPRITE_SIZE * x);
	else if (game->map->map[x][y] == 'C')
	{
		mlx_put_image_to_window (game->mlx, game->win,
			game->img->ground, SPRITE_SIZE * y, SPRITE_SIZE * x);
		mlx_put_image_to_window (game->mlx, game->win,
			game->img->col, SPRITE_SIZE * y, SPRITE_SIZE * x);
	}
	else if (game->map->map[x][y] == 'E')
		mlx_put_image_to_window (game->mlx, game->win,
			game->img->exit, SPRITE_SIZE * y, SPRITE_SIZE * x);
